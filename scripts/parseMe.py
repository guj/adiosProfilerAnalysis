...
parser.add_argument(
    ...
    "Will be accessed as ["
    + TAGS["input_dir"]
    + "]/[ioType]_<secs/nCalls>_[profileTag]"
...
if not os.path.isdir(command_options[TAGS["input_dir"]]):
    parser.error(
        f"Input directory '{command_options[TAGS['input_dir']]}' does not exist"
    )
...
try:
    ...
    if command_options[TAGS["zero"]] < 0:
        parser.error(
            f"Zero threshold ({command_options[TAGS['zero']]}) must be non-negative"
        )
except ValueError:
    parser.error(
        f"Zero threshold must be a number, got '{command_options[TAGS['zero']]}'"
    )
...
if level_val not in ("true", "false"):
    parser.error(
        f"Level must be 'True' or 'False', got '{command_options[TAGS['level']]}'"
    )
